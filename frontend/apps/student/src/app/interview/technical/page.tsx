...
const TechnicalRoundWorkspace = dynamic(() => import("@/components/TechnicalRoundWorkspace"), {
  ssr: false,
  loading: () => (
...
export default function TechnicalRoundPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = (await interviewService.initTechnical(session.sessionId)) as {
          sessionToken?: string;
  ...
  if (state === "error") {
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-slate-950 text-center px-6 gap-3">
        ...
        <p className="text-slate-200 text-sm">Couldn&apos;t start the technical round</p>
